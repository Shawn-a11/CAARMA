...
class PersistentSynthState:
    ...
    def rebuild_pairing(self, W):
        ...
        # Reserve coverage pairs before the remaining candidates. A plain
        # global sort can spend a small registry entirely on low-index pairs,
        # leaving some anchors without any assigned column. Coverage-first
        # reservation keeps synth_max_factor=1 usable while preserving a
        # deterministic pair->column map on every DDP rank. Reserving a column
        # does not create a CRP class; creation still requires a committed visit.
        covered_speakers = set()
        for key in self.pair_col:
            covered_speakers.update(self._members(key))

        reservation_order = []
        reservation_set = set()
        for speaker in range(self.num_real):
            if speaker in covered_speakers:
                continue
            candidates = self.candidate_pairs.get(speaker, [])
            if not candidates:
                continue
            key = candidates[0]
            if key not in self.pair_col and key not in reservation_set:
                reservation_order.append(key)
                reservation_set.add(key)
            covered_speakers.update(self._members(key))
