...
internally consistent 3.09 recipe. These differences must be resolved with the
historical PSC assets before claiming reproduction.
...
| Vox1 utterances | manifest not published | table says 153,516 utterances but 1,211 speakers | recover historical CSV; otherwise audit official dev-only manifest separately |
...
Those two values need historical-manifest verification because the public
repository does not include the CSV. The PSC recovery job therefore searches
the original author directory before constructing a replacement manifest.

## Reproduction Order

1. Recover and hash the historical config, manifest, and trials if readable.
2. Run a 12-step single-GPU source smoke.
3. Run the public-source schedule on four V100-32 GPUs with the recovered data.
