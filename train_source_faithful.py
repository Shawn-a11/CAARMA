"""4-GPU DDP training entry that faithfully mirrors the original CAARMA repo
...
class Task(LightningModule):
    def __init__(self, features, model, loss, config, learning_rate=0.2,
        ...
        # d_step_counter / g_step_counter created lazily in training_step
    ...
    def _d_step(self, opt_d, waveform, label):
        ...
        # Source: /2 (paper Eq.1 does not divide; source does)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ── source state-machine counters (lazy init, exact source logic) ──
        if not hasattr(self, 'd_step_counter'):
            self.d_step_counter = 0
        if not hasattr(self, 'g_step_counter'):
            self.g_step_counter = 0

        # Source reset conditions: 5G+1D in pretrain → reset; 1G+1D post → reset.
        ...
        # ─── PRETRAIN PHASE (epoch ≤ 15): 5 G updates per 1 D update ───
        if self.current_epoch <= self.pretrain_eps:
            if self.g_step_counter < 5:
                loss = self._g_step(opt_main, waveform, label,
                                    lambda_adv_value=0.0005, adjust=False)
                self.g_step_counter += 1
                return loss
            elif self.d_step_counter < 1:
                loss = self._d_step(opt_d, waveform, label)
                self.d_step_counter += 1
                return loss

        # ─── POST-PRETRAIN PHASE (epoch > 15): 1 D then 1 G cycle ───
        else:
            if self.d_step_counter < 1:
                ...
                loss = self._g_step(opt_main, waveform, label,
                                    lambda_adv_value=0.25, adjust=True)
                self.g_step_counter += 1
                return loss
