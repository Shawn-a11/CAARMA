...
class PscProjectionCrpPortTest(unittest.TestCase):
    def test_method_config(self):
        config = yaml.safe_load((ROOT / "config_psc_projection_crp.yaml").read_text())
        ...
        self.assertEqual(config["title"], "projection_crp_persistent_psc")
    ...
    def test_slurm_entrypoint(self):
        script = (ROOT / "scripts/psc/train_projection_crp.slurm").read_text()
        self.assertIn("config_psc_projection_crp.yaml", script)
