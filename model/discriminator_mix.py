...
class ProjectionDiscriminator_spectral(nn.Module):
    """Projection MLP-D conditioned on the assigned class prototype."""
    ...
    def forward(self, x, condition):
        if condition is None:
            raise ValueError(
                "ProjectionDiscriminator_spectral requires a condition tensor"
            )
        phi = self.activation(self.embed(x))
        psi = self.cond(F.normalize(condition, dim=1))
        compatibility = (phi * psi).sum(dim=1, keepdim=True) / self.scale
        return self.head(phi) + compatibility


class ConcatConditionDiscriminator_spectral(nn.Module):
    """Parameter-matched conditional MLP over concatenated (e, q)."""
    ...
    def forward(self, x, condition):
        if condition is None:
            raise ValueError(
                "ConcatConditionDiscriminator_spectral requires a condition tensor"
            )
        condition = F.normalize(condition, dim=1)
        hidden = self.activation(
            self.fc1(torch.cat([x, condition], dim=1))
        )
        return self.fc2(hidden)


class QOnlyConditionDiscriminator_spectral(nn.Module):
    """Leakage control that predicts real/fake from q while ignoring e."""
    ...
    def forward(self, x, condition):
        if condition is None:
            raise ValueError(
                "QOnlyConditionDiscriminator_spectral requires a condition tensor"
            )
        hidden = self.activation(self.fc1(F.normalize(condition, dim=1)))
        return self.fc2(hidden)
